...
class _VoxImagePanel(QGraphicsView):
	...
	def __init__(self):
		...
		self.npImage = None
		self.imageFormat = None      # either 'float' or 'int'
	...
	def _updateImageLUT(self):
		...
		im8 = ((self.npImage.astype(np.float32) - self.winMin) / (self.winMax \
				 - self.winMin + np.finfo(float).eps)) * 255.0
		im8[im8 > 255.0] = 255.0
		im8[im8 < 0.0] = 0.0
		im8 = im8.astype(np.uint8)		

		self.qImage = QImage(im8.tobytes(order='C'), self.npImage.shape[1], \
			self.npImage.shape[0], self.npImage.shape[1], QImage.Format_Grayscale8)
	...
	def changeImage(self, npImage):
		""" Change the underlying matrix of the image without changing zoom, LUT and position.
			NOTE: to be called only if you're sure that the size of the input image is the same
				  of the current image 
		"""
		# Set the new numpy image:
		self.npImage = npImage
		if ((npImage.dtype == np.float32) or (npImage.dtype == np.float64)):
			self.imageFormat = 'float' 
		else:
	...
	def mouseMoveEvent(self, event):
		...
		if self.npImage is not None:
			...
			if ((x >= 0) and (x <= (self.npImage.shape[1] - 1)) and \
				...
				self.mouseHoverEvent.emit(x, y, self.npImage[y, x], self.imageFormat)
			else:
